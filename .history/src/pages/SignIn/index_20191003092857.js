...
export default function SignIn() {
  ...
  async function handleSubmit({ cpf, password }) {
    ...
    if (await (response.data.payload.token !== '')) {
      await localStorage.setItem('data', response.data.payload);
      await localStorage.setItem('token', response.data.payload.token);
      api.defaults.headers.Authorization = await `X-Token ${response.data.payload.token}`;
