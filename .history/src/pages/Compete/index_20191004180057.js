...
export default function Compete() {
  ...
  async function handleSearch(e) {
    if (await (e.length > 5)) {
      ...
      await setPeople(response.data.payload.elogios[0]);
    }
  }

  async function handleSerachId(e) {
    const elogio = await api.get(`api/v1/pessoa/elogio/id/${e}`);
    await setElogios(elogio.data.payload);
  ...
  return (
    <>
      ...
      <Container>
        {elogios &&
          people.map(item => (
            <List key={item.idpessoa} onClick={handleSerachId(item.idpessoa)}>
              {item.Graduacao.graduacao} {item.rg} {item.nome} -{' '}
              {item.SiglaUnidade}
            </List>
          ))}
        {elogios.map(item => (
          <List key={item.idelogio} onClick={handleSerachId(item.idpessoa)}>
            {item.} {item.rg} {item.nome} -{' '}
            {item.SiglaUnidade}
          </List>
        ))}
      </Container>
