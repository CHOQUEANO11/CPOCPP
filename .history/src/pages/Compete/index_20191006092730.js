...
export default function Compete() {
  ...
  async function listItems(items, pageActual, limitItems) {
    const result = [];
    const totalPages = await Math.ceil(items.length / limitItems);
    let count = (await pageActual) * limitItems - limitItems;
    const delimiter = (await count) + limitItems;
    ...
    return setTotalPage(result) console.log(TotalPage);
  }
  ...
  async function handleSearch(e) {
    if (await (e.length > 5)) {
      ...
      await listItems(response.data.payload, 1, 10);
    }
  }
