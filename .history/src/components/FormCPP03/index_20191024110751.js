...
export default function Potentials() {
  ...
  const [cfpstate, setCfpState] = useState({
    checkedA: [2, 0],
    checkedB: [1, 5]
  });
  ...
  async function handleResultTefpoi(e) {
    const result = e.target.value;
    await setTefpoiQtd(result);
    await setTefpoiResult(parseFloat((0.4 * result).toFixed(2)));
  }

  async function handleResultTefpnp(e) {
    const result = e.target.value;
    await setTefpnpQtd(e.target.value);
    await setTefpnpResult(parseFloat((0.2 * result).toFixed(2)));
  }
      ...
      <Form>
        ...
        <form className={classes1.container} noValidate autoComplete="off">
          ...
          <TextField
            ...
            className={classes1.textField}
            value={tefpoiQtd}
            onChange={e => setTefpoiQtd(e.target.value) + setTefpoiResult(parseFloat((0.4 * result).toFixed(2)));}
            margin="normal"
            variant="outlined"
          />
          <TextField
        ...
        <h4 style={{ textAlign: 'center' }}>
          2.1 - Habilitação para o exercício da docência e/ou policial-militar -
          30h/a
        </h4>
        <form className={classes1.container} noValidate autoComplete="off">
          <TextField
        ...
        <Grid container spacing={1}>
          <Grid item xs={6}>
            <Paper className={classes.paper}>
              CURSO DE APERFEIÇOAMENTO DE SARGENTO(CAS): MB:{' '}
              <Switch
          ...
          <Grid item xs={6}>
            <Paper className={classes.paper}>
              CURSO DE FORMAÇÃO DE PRAÇAS: MB:{' '}
              <Switch
