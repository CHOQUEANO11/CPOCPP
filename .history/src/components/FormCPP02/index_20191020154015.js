...
export default function FormCPP02({ person, showForm }) {
  ...
  useEffect(() => {
    ...
    async function handleGetForm() {
      const response = apiInternal.get('formcpp02');

      setTeste(JSON.parse(response.data[0].lav));
      console.log(JSON.parse(response.data[0].lav));
    }
    const date = new Date();
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={2}>
              <Paper className={classes.paper}>
                GRAD: {person.Graduacao.graduacao}
              </Paper>
            </Grid>
            <Grid item xs={2}>
